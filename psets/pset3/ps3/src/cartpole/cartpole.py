...
def choose_action(state, mdp_data):
    ...
    transition_probabilities = mdp_data['transition_probs']
    state_values = mdp_data['value']

    value_for_action_0 = np.dot(transition_probabilities[state, :, available_actions[0]], state_values)
    value_for_action_1 = np.dot(transition_probabilities[state, :, available_actions[1]], state_values)

    if value_for_action_1 > value_for_action_0:
        selected_action = available_actions[1]
    elif value_for_action_0 > value_for_action_1:
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    available_actions = [0, 1]
    transition_probabilities = mdp_data['transition_probs']
    state_count = mdp_data['num_states']
    reward_count_data = mdp_data['reward_counts']
    transition_count_data = mdp_data['transition_counts']
    rewards = mdp_data['reward']

    for action in available_actions:
        for state in range(state_count):
            total_transitions = np.sum(transition_count_data[state, :, action])

            if total_transitions > 0:
                transition_probabilities[state, :, action] = (
                        transition_count_data[state, :, action] / total_transitions
                )

    for state in range(state_count):
        if reward_count_data[state, 1] > 0:
            rewards[state] = reward_count_data[state, 0] / reward_count_data[state, 1]
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    state_count = mdp_data['num_states']
    transition_probabilities = mdp_data['transition_probs']
    action_count = transition_probabilities.shape[2]

    iter_count = 0
    while True:
        updated_values = np.array([
            np.max(
                [transition_probabilities[state, :, action].dot(mdp_data['value']) for action in range(action_count)])
            for state in range(state_count)
        ])
        updated_values = mdp_data['reward'] + gamma * updated_values

        difference = np.max(np.abs(updated_values - mdp_data['value']))

        mdp_data['value'] = updated_values

        iter_count += 1
        if difference < tolerance or iter_count > 1:
            break
